...
{
  printf '# 5-14-26 Qwen-Scope Workspace\n\n'
  printf 'This is the fresh 5-14 Qwen-Scope workspace for Transformers/PyTorch residual-stream trajectory capture and Qwen-Scope SAE analysis.\n\n'
  printf 'It is observational only for this run: no steering, no Hauhau, no llama.cpp, no GGUF routing work, no all-layer expansion, and no semantic SAE feature labels.\n\n'
  printf '- model_repo: `%s`\n' "$MODEL_REPO"
  printf '- model_path: `%s`\n' "$MODEL_PATH"
  printf '- sae_repo: `%s`\n' "$SAE_REPO"
  printf '- sae_path: `%s`\n' "$SAE_PATH"
