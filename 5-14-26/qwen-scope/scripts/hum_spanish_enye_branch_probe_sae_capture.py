#!/usr/bin/env python3
"""Hum-prompt Spanish enye-control branch-probing Qwen-Scope SAE trajectory capture."""
...
PROMPT_TSV_PATH = ROOT / "prompts" / "hum_spanish_enye_branch_probe_prompts.tsv"
SCRIPT_PATH = ROOT / "scripts" / "hum_spanish_enye_branch_probe_sae_capture.py"
OUT_DIR = ROOT / "sae_outputs" / "hum_spanish_enye_branch_probe_sae_capture"
PROVENANCE_PATH = ROOT / "provenance" / "hum_spanish_enye_branch_probe_sae_capture_20260514.txt"
...
BASE_CONDITIONS = ["ascii_control", "n_all", "n_high_impact"]
BRANCH_LABELS = [
    "greedy_no_prefix",
    "prefix_no",
    "prefix_yes",
...
def substitution_counts(prompt_rows: list[dict[str, str]]) -> dict[str, int]:
    counts = {}
    for condition in BASE_CONDITIONS:
        base = next(row["base_prompt_text"] for row in prompt_rows if row["condition_family"] == condition)
        counts[condition] = base.count("ñ")
    return counts
...
def write_summary(
    ...
    lines.extend(["", "## Checking Prefix Control", ""])
    checking_rows = {
        row["condition_family"]: row
        for row in generated_rows
        if row["branch_label"] == "prefix_checking"
    }
    for condition in BASE_CONDITIONS:
        row = checking_rows[condition]
        cls = classify_generated_start(row["forced_prefix"], row["generated_text_start"])
        lines.append(f"- `{condition}` + `Checking...`: class={cls['generated_start_class']}; start={clean_cell(row['generated_text_start'])}")

    ascii_checking_class = classify_generated_start(
        checking_rows["ascii_control"]["forced_prefix"],
        checking_rows["ascii_control"]["generated_text_start"],
    )["generated_start_class"]
    n_checking_classes = {
        condition: classify_generated_start(
            checking_rows[condition]["forced_prefix"],
            checking_rows[condition]["generated_text_start"],
        )["generated_start_class"]
        for condition in ["n_all", "n_high_impact"]
    }
    n_reproduces_prior_d_flip = any(
        "the hum is present" in checking_rows[condition]["generated_text_start"].lower()
        for condition in ["n_all", "n_high_impact"]
    )
    if all(value == ascii_checking_class for value in n_checking_classes.values()):
        checking_relation = "n_to_enye behaved more like ASCII than the prior d-stroke Checking result by this string heuristic."
    elif n_reproduces_prior_d_flip:
        checking_relation = "n_to_enye reproduced the prior d-stroke-style Checking flip by producing `The hum is present...` language."
    else:
        checking_relation = "n_to_enye differed from ASCII on Checking, but did not reproduce the exact prior d-stroke `The hum is present...` start."

    lines.extend(["", "## Comparison To Prior D-Stroke Checking Result", ""])
    lines.append("- Prior d-stroke result: `ascii_control + Checking...` -> denial; `d_all / d_high_impact + Checking...` -> `The hum is present...`.")
    lines.append(f"- This run: `n_all + Checking...` class={n_checking_classes['n_all']}; `n_high_impact + Checking...` class={n_checking_classes['n_high_impact']}.")
    lines.append(f"- Evidence relation: {checking_relation}")
    lines.append("- SAE feature-ID cross-run comparison was not performed in this script; this section compares generated behavior only.")
...
def main() -> None:
    ...
    n_to_enye_counts = substitution_counts(prompt_rows)
    n_high_impact_materially_distinct_from_n_all = (
        next(row["base_prompt_text"] for row in prompt_rows if row["condition_family"] == "n_all")
        != next(row["base_prompt_text"] for row in prompt_rows if row["condition_family"] == "n_high_impact")
    )
    ...
    summary_metrics = write_summary(
        OUT_DIR / "hum_spanish_enye_branch_probe_summary.md",
        next_token_rows,
        generated_rows,
        comparison_rows,
        skipped_positions,
    ...
    metadata = {
        ...
        "control_terminology": "n_to_enye / spanish_enye_control; ASCII is the true no-diacritic control.",
        "n_to_enye_substitution_counts": n_to_enye_counts,
        "n_high_impact_materially_distinct_from_n_all": n_high_impact_materially_distinct_from_n_all,
    ...
    (OUT_DIR / "hum_spanish_enye_branch_probe_metadata.json").write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    provenance_lines = [
        ...
        f"n_to_enye_substitution_counts={json.dumps(n_to_enye_counts, sort_keys=True)}",
        f"n_high_impact_materially_distinct_from_n_all={str(n_high_impact_materially_distinct_from_n_all).lower()}",
        ...
        "outputs_written=" + ",".join(
            ...
            for path in [
                ...
                OUT_DIR / "trajectory_feature_presence_summary.tsv",
                OUT_DIR / "branch_probe_comparison.tsv",
                OUT_DIR / "hum_spanish_enye_branch_probe_metadata.json",
                OUT_DIR / "hum_spanish_enye_branch_probe_summary.md",
            ]
        ),
        "selected_layer_hooks_used=true",
    ...
    print(
        json.dumps(
            {
                "status": "hum_spanish_enye_branch_probe_sae_capture_complete",
                "topk_row_count": len(topk_rows),
                "next_token_logit_row_count": len(next_token_rows),
                "generated_text_row_count": len(generated_rows),
                "skipped_position_count": len(skipped_positions),
                "metadata_path": str(OUT_DIR / "hum_spanish_enye_branch_probe_metadata.json"),
