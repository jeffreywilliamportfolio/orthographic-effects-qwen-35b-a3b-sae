...

{
  ...
  printf 'purpose=%s\n' '5-14 hum Spanish enye-control branch-probing SAE trajectory setup'
  printf 'model_repo=%s\n' "$MODEL_REPO"
  printf 'model_path=%s\n' "$MODEL_PATH"
  printf 'sae_repo=%s\n' "$SAE_REPO"
  printf 'sae_path=%s\n' "$SAE_PATH"
  printf 'confirmation=%s\n' 'setup only; no experiment run in this script'
} > "$PROV_DIR/setup_hum_spanish_enye_branch_probe_sae_workspace_20260514.txt"
...
python -m pip freeze > "$PROV_DIR/pip_freeze_hum_spanish_enye_branch_probe_sae_20260514.txt"
python - <<'PY' > "$PROV_DIR/python_environment_snapshot_hum_spanish_enye_branch_probe_sae_20260514.txt"
import json, platform, subprocess, sys
import torch
snapshot = {
    "python": sys.version,
...
find "$MODEL_PATH" -type f -printf '%P\t%s\n' | sort > "$MANIFEST_DIR/base_model_file_manifest_hum_spanish_enye_branch_probe_sae.tsv"
find "$SAE_PATH" -type f -printf '%P\t%s\n' | sort > "$MANIFEST_DIR/sae_file_manifest_hum_spanish_enye_branch_probe_sae.tsv"
find "$SAE_PATH" -maxdepth 2 -type f | sort > "$MANIFEST_DIR/sae_find_layout_hum_spanish_enye_branch_probe_sae.txt"

{
  printf '%s\n\n' '# 5-14-26 Qwen-Scope Hum Spanish Enye-Control Branch-Probe SAE Workspace'
  printf '%s\n\n' 'Fresh remote workspace for hum-prompt Spanish enye-control branch-probing SAE trajectory capture.'
  printf '%s\n\n' 'This is branch probing / prefix intervention only. It is not residual steering and not SAE feature steering. It uses Transformers/PyTorch selected-layer capture plus Qwen-Scope TopK-50 SAE encoding.'
...
} > "$MANIFEST_DIR/workspace_manifest_hum_spanish_enye_branch_probe_sae.md"
...
} > "$PROV_DIR/setup_hum_spanish_enye_branch_probe_sae_download_verification_20260514.txt"

df -h /workspace > "$PROV_DIR/disk_after_hum_spanish_enye_branch_probe_sae_downloads_20260514.txt"
echo "HUM_SPANISH_ENYE_BRANCH_PROBE_SAE_SETUP_COMPLETE"
