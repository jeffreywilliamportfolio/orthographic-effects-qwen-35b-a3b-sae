...

{
  ...
  printf 'purpose=%s\n' '5-14 e_only prefix intervention setup'
  printf 'model_repo=%s\n' "$MODEL_REPO"
  printf 'model_path=%s\n' "$MODEL_PATH"
  printf 'sae_repo=%s\n' "$SAE_REPO"
  printf 'sae_path=%s\n' "$SAE_PATH"
  printf 'confirmation=%s\n' 'setup only; no experiment run in this script'
} > "$PROV_DIR/setup_prefix_intervention_workspace_20260514.txt"
...
python -m pip freeze > "$PROV_DIR/pip_freeze_prefix_intervention_20260514.txt"
python - <<'PY' > "$PROV_DIR/python_environment_snapshot_prefix_intervention_20260514.txt"
import json, platform, subprocess, sys
import torch
snapshot = {
    "python": sys.version,
...
find "$MODEL_PATH" -type f -printf '%P\t%s\n' | sort > "$MANIFEST_DIR/base_model_file_manifest_prefix_intervention.tsv"
find "$SAE_PATH" -type f -printf '%P\t%s\n' | sort > "$MANIFEST_DIR/sae_file_manifest_prefix_intervention.tsv"
find "$SAE_PATH" -maxdepth 2 -type f | sort > "$MANIFEST_DIR/sae_find_layout_prefix_intervention.txt"
...
} > "$MANIFEST_DIR/workspace_manifest_prefix_intervention.md"
...
} > "$PROV_DIR/setup_prefix_intervention_download_verification_20260514.txt"

df -h /workspace > "$PROV_DIR/disk_after_prefix_intervention_downloads_20260514.txt"
echo "PREFIX_SETUP_COMPLETE"
