#!/usr/bin/env python3
"""Hum-prompt d-diacritic 128-token Qwen-Scope SAE trajectory capture."""
...
PROMPT_TSV_PATH = ROOT / "prompts" / "hum_d_diacritic_128_sae_prompts.tsv"
SCRIPT_PATH = ROOT / "scripts" / "hum_d_diacritic_128_sae_capture.py"
OUT_DIR = ROOT / "sae_outputs" / "hum_d_diacritic_128_sae_capture"
PROVENANCE_PATH = ROOT / "provenance" / "hum_d_diacritic_128_sae_capture_20260514.txt"
...
def read_prompts() -> list[dict[str, str]]:
    with PROMPT_TSV_PATH.open(newline="", encoding="utf-8") as f:
        ...
        rows = list(reader)
    if len(rows) != 3:
        raise ValueError(f"Expected exactly 3 prompt rows, found {len(rows)}")
...
def main() -> None:
    ...
    summary_metrics = write_summary(OUT_DIR / "hum_d_diacritic_128_sae_summary.md", generated_rows, topk_rows, presence_rows, skipped_positions)
    ...
    (OUT_DIR / "hum_d_diacritic_128_sae_metadata.json").write_text(json.dumps(metadata, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    provenance_lines = [
        ...
        "outputs_written=" + ",".join(
            ...
            for path in [
                ...
                OUT_DIR / "hum_d_diacritic_128_sae_metadata.json",
                OUT_DIR / "hum_d_diacritic_128_sae_summary.md",
            ]
        ),
        "selected_layer_hooks_used=true",
    ...
    print(
        json.dumps(
            {
                "status": "hum_d_diacritic_128_sae_capture_complete",
                "topk_row_count": len(topk_rows),
                "generated_text_row_count": len(generated_rows),
                "skipped_position_count": len(skipped_positions),
                "metadata_path": str(OUT_DIR / "hum_d_diacritic_128_sae_metadata.json"),
